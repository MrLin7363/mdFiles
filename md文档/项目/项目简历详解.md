...
​	为什么一分钟两次，可以容忍这么多的可用性丢失么？ 1.应对演练够用了  2. redis集群这种情况基本不会发生，本身redis集群就是高可用，时间当然可以设置短一些，这样可以减少性能损耗 3. 时间太短可能不是真的有问题，可能网络波动，太长就达不到可用性效果
...
按照订单group分组后，每个组一个发货单,相当于一个组可能有多个子订单，每个子订单对应一个发货单
发货单状态  -> 初始发货单
子订单状态  >  已拆单
...
表：支付流水记录  退款流水记录   支付配置表   支付退款不成功回调表   支付只能选择一个渠道，前端用户点支付的时候会跳到微信，不会重复支付
...
```
交易中心 /cancelOrder 判断支付完成才会调支付网关退款
-> 支付网关refund 接口  支付单号，退款单号-交易中心生成，退款金额，回调交易中心URL
...
或者悲观锁 for update  锁支付记录，将查询所有记录和校验的逻辑放到一个@Transactional 里
...
```
 /**
    ...
    public String deductStock(Integer productId) {
       // 查询支付记录， for update 并锁住,让同一个订单只能同时有一个退款请求在执行
    	
       // 退款金额校验
       // 查询所有退款流水PDNGING+SUCCESS
       // 如果退款流水总金额 + 此次请求退款金额 > 订单总金额 ，直接异常
...
```
   private RefundOrder checkAndBuildRefund(RefundPaymentCmd cmd,Payment payment){
        ...
        // 查到的同时需要锁定，这里需要对支付流水加分布式锁，不可以让同一个支付流水同时多个退款申请同时进行
